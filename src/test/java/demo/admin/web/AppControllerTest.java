...
/* ExtendWith
    - 테스트를 진행시킬 때 Junit에 내장된 실행자 외에 다른 실행자를 실행시킨다.
    - 여기서는 SpringExtension라는 스프링 실행자를 사용한다.
...
public class AppControllerTest {
    ...
    public void return_helloDto() throws Exception{
        ...
        /* param
            ...
            - 단, 값은 String만 허용됨
            - 그래서 숫자/날짜 등의 데이터도 등록할때는 문자열로 변경해야만 가능.

           jsonPath
            - JSON 응답값을 필드별로 검증할 수 있는 메소드
            - $를 기준으로 필드명을 명시
            - 여기서는 name과 amount를 검증하므로 $.name, $.amount로 검증.
         */
        mvc.perform(
                get("/hello/dto")
                        .param("name", name)
